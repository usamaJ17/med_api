                            ...
                            <table class="table border-no" id="example1">
                                ...
                                @foreach ($medicals as $medical)
                                    <tr class="hover-primary">
                                        <td><a href="{{ route('medical.show', $medical->id) }}">
                                                #p-DH-{{ $medical->id }} </a></td>
                                        <td><a href="{{ route('medical.show', $medical->id) }}">
                                        ...
                                        <td>{{ optional(optional($medical->professionalDetails)->professions)->name ?? 'N/A' }}</td>
                                        <td>{{ optional(optional($medical->professionalDetails)->ranks)->name ?? 'N/A' }}</td>
                                        <td>{{ Carbon::parse($medical->created_at)->format('d/m/Y') }}</td>
                                            ...
                                            <div class="btn-group">
                                                ...
                                                <div class="dropdown-menu">
                                                    <a class="dropdown-item"
                                                       href="{{ route('medical.show', $medical->id) }}">View Details</a>
                                                    @if(auth()->user()->hasRole('admin'))
                                                        <a class="dropdown-item"
                                                           onclick="DeleteRecord({{ $medical->id }})">Delete</a>
                                                    @endif
                                                </div>
                                            </div>
                                        </td>
                ...
                <div class="modal-body">
                    <form action="{{ route('medical.export') }}" method="GET">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" multiple="" name="fields[]"
                                    data-placeholder="Select Columns" style="width: 100%;" aria-hidden="true">
